...
public:
    ...
    {
        EXPECT_CALL(*fontMock, getTexture(_)).Times(times).WillRepeatedly(Return(texture));
    }

    void expectGetGlyphRepeatedly()
    {
        EXPECT_CALL(*fontMock, getGlyph(_, _, _, _)).WillRepeatedly(ReturnRef(t_glyph1));
    }
    ...
    void expectedUpdateVertexArray(const unsigned int times = 1)
    {
        ...
        EXPECT_CALL(*fontMock, getUnderlinePosition(_)).WillRepeatedly(Return(1.f));
        EXPECT_CALL(*fontMock, getUnderlineThickness(_)).WillRepeatedly(Return(2.f));
        expectGetGlyphRepeatedly();
        EXPECT_CALL(*fontMock, getLineSpacing(_)).WillRepeatedly(Return(1.f));
        EXPECT_CALL(*fontMock, getKerning(_, _, _, _)).WillRepeatedly(Return(1.f));
...
TEST_F(TextTests, setFont_whenSetFont_shouldSetFontAndUpdateVertexArray)
{
...
TEST_F(TextTests, setColor_whenSetColorToBlack_shouldSetColorAndUpdateVertexArray)
{
    expectedUpdateVertexArray();
