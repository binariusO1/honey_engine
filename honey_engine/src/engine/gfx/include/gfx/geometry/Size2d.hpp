...
public:
    ...
    constexpr Size2d(T A, T B) : width(A), height(B) {}
    inline bool isEmpty() const { return (width==0 and height==0) ? true : false; }
    template <typename U>
    constexpr explicit Size2d(const Size2d<U>& size) : width(static_cast<T>(size.width)), height(static_cast<T>(size.height)) {}
